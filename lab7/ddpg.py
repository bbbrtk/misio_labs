...
class ActorCritic(object):
	def __init__(self, env_to_use):
		# init hyperparamas
		self.n_train_games = 100
		self.dropout_actor = 0.4			# dropout rate for actor (0 = no dropout)
		self.dropout_critic = 0.4			# dropout rate for critic (0 = no dropout)
		self.num_episodes = 10000		# number of episodes, default: 15 000
		self.max_steps_ep = 10000	# default 10000 !!!! max number of steps per episode (unless env has a lower hardcoded limit)
		self.tau = 1e-2				# soft target update rate
		self.replay_memory_capacity = int(1e5)	# capacity of experience replay memory
		...
		# init environment
		self.PATH = 'tmpBipedal/'
		self.env_to_use = env_to_use
		self.env = gym.make(env_to_use)
		self.state_dim = np.prod(np.array(env.observation_space.shape)) 	# Get total number of dimensions in state
		self.action_dim = np.prod(np.array(env.action_space.shape))		# Assuming continuous action space
	...
	def generate_actor_network(self, s, trainable, reuse):
		...
		hidden_drop = tf.layers.dropout(hidden, rate = self.dropout_actor, training = trainable & self.is_training_ph)
		hidden_2 = tf.layers.dense(hidden_drop, 8, activation = tf.nn.relu, trainable = trainable, name = 'dense_1', reuse = reuse)
		hidden_drop_2 = tf.layers.dropout(hidden_2, rate = self.dropout_actor, training = trainable & self.is_training_ph)
		hidden_3 = tf.layers.dense(hidden_drop_2, 8, activation = tf.nn.relu, trainable = trainable, name = 'dense_2', reuse = reuse)
		hidden_drop_3 = tf.layers.dropout(hidden_3, rate = self.dropout_actor, training = trainable & self.is_training_ph)
		actions_unscaled = tf.layers.dense(hidden_drop_3, self.action_dim, trainable = trainable, name = 'dense_3', reuse = reuse)
		actions = env.action_space.low + tf.nn.sigmoid(actions_unscaled)*(self.env.action_space.high - self.env.action_space.low) # bound the actions to the valid range
		return actions
	...
	def generate_critic_network(self, s, a, trainable, reuse):
		...
		hidden_drop = tf.layers.dropout(hidden, rate = self.dropout_critic, training = trainable & self.is_training_ph)
		hidden_2 = tf.layers.dense(hidden_drop, 8, activation = tf.nn.relu, trainable = trainable, name = 'dense_1', reuse = reuse)
		hidden_drop_2 = tf.layers.dropout(hidden_2, rate = self.dropout_critic, training = trainable & self.is_training_ph)
		hidden_3 = tf.layers.dense(hidden_drop_2, 8, activation = tf.nn.relu, trainable = trainable, name = 'dense_2', reuse = reuse)
		hidden_drop_3 = tf.layers.dropout(hidden_3, rate = self.dropout_critic, training = trainable & self.is_training_ph)
	...
	def makeSteps(self):
		...
		for ep in range(num_episodes):
			...
			self.noise_scale = (self.initial_noise_scale * self.noise_decay**ep) * (self.env.action_space.high - self.env.action_space.low)
			...
			for t in range(max_steps_ep):
				...
				action_for_state, = sess.run(self.actions, 
					feed_dict = {state_ph: self.observation[None], self.is_training_ph: False})
				...
				action_for_state += self.noise_scale*noise_process

				# take step
				next_observation, reward, done, _info = self.env.step(action_for_state)
				total_reward += reward

				add_to_memory((self.observation, action_for_state, reward, next_observation, 
	...
	def trainingLoop(self):
		...
		rewards = []
		for i_episode in trange(self.n_train_games):
			...
			while not done:
				...
				action_for_state, = sess.run(self.actions, feed_dict = {state_ph: self.observation[None], is_training_ph: False})
				noise_process = exploration_theta*(exploration_mu - noise_process) + exploration_sigma*np.random.randn(action_dim)
				action_for_state += self.noise_scale*noise_process
