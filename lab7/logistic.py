...
class LogisticPolicy:
    ...
    def update(self, rewards, obs, actions):
        ...
        assert grad_log_p.shape == (len(obs), NUM)
...
def train(θ, α, γ, Policy, MAX_EPISODES=1000, seed=None, evaluate=False):
    ...
    if evaluate:
        env = Monitor(env, 'pendulum/', video_callable=False, force=True)
        for _ in range(100):
...
episode_rewards, policy = train(θ=np.random.rand(NUM),
                                α=0.002,
                                γ=0.99,
